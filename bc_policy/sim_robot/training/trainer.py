...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Geometric rotation loss weight applied to the flow-predicted action endpoint.",
    )
    parser.add_argument(
        "--visual-rotation-loss-weight",
        type=float,
        default=0.0,
        help="Image-only geometric rotation loss weight for predicting target rot6d.",
    )
    ...
    action_center, action_scale = normalizer._params_numpy("action")
    policy_config = SimFlowMatchingConfig(
        ...
        action_normalizer_center=tuple(float(value) for value in action_center),
        action_normalizer_scale=tuple(float(value) for value in action_scale),
    )
