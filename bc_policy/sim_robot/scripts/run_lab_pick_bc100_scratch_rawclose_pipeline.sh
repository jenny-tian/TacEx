...
RECORDS=datasets/lab_pick_slide_bc100_diverse_safe70_overforce24_pos6_xypm10_yawpm45
SOURCE50=datasets/lab_pick_slide_bc50_diverse_safe35_overforce12_pos3_xypm10_yawpm45
DATASET=/dev/shm/tacex_lab_pick_bc100_diverse_safe70_overforce24_pos6.hdf5
OUTPUT=outputs/lab_pick_flow_bc100_scratch_rawclose_safe70_overforce24_pos6

mkdir -p "${RECORDS}" "${OUTPUT}"

if [[ $(find "${RECORDS}" -maxdepth 1 -type d -name 'record_*' | wc -l) -eq 0 ]]; then
  cp -a "${SOURCE50}"/record_* "${RECORDS}/"
fi

current=$(find "${RECORDS}" -maxdepth 1 -type d -name 'record_*' | wc -l)
if (( current < 100 )); then
  PYTHONPATH=source/tacex:source/tacex_assets:source/tacex_tasks \
    "${PYTHON}" scripts/demos/lab_pick/collect_bc_dataset.py \
    --labware slide --num_envs 1 --num_demos "$((100 - current))" --max_attempts 300 \
    --record_dir "${RECORDS}" --seed 10000 --safe_demo_fraction 0.70 \
...
"${PYTHON}" bc_policy/sim_robot/scripts/convert_records_to_hdf5.py \
  --input "${RECORDS}" --output "${DATASET}" --max-episodes 100 \
...
/usr/bin/env -u LD_PRELOAD -u VGL_ISACTIVE -u VGL_DISPLAY -u DISPLAY \
  ...
  --epochs 60 --batch-size 32 --num-workers 4 \
