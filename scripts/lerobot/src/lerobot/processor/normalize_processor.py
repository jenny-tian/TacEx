    ...
    **Stats Override Preservation:**
    ...
    def _normalize_observation(self, observation: dict[str, Any], inverse: bool) -> dict[str, Tensor]:
        ...
        for key, feature in self.features.items():
            if self.normalize_observation_keys is not None and key not in self.normalize_observation_keys:
                continue
            # Skip ACTION and LANGUAGE types - ACTION is handled separately, LANGUAGE is string type
            if feature.type in (FeatureType.ACTION, FeatureType.LANGUAGE):
                continue
            if key in new_observation:
                value = new_observation[key]
                # Skip string types (e.g., observation.language) as they cannot be converted to tensors
                # Note: observation.language may be incorrectly classified as STATE type, so we check the actual value type
                if isinstance(value, str) or (isinstance(value, (list, tuple)) and len(value) > 0 and isinstance(value[0], str)):
                    continue
    ...
    def _apply_transform(
        ...
        if norm_mode == NormalizationMode.MEAN_STD:
            ...
            denom = std + self.eps
            if inverse:
                return tensor * denom + mean #modified by Kun Song
