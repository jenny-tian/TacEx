...
class DINOv3ACTBackbone(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "DINOACTRTC requires the `transformers` package. Install with: pip install 'lerobot[transformers-dep]'"
            ) from e

        local_files_only = _env_flag("VLARL_DINOV3_LOCAL_FILES_ONLY", True)
        try:
            ...
            if local_files_only:
                raise OSError(
                    f"Could not load DINOv3 model {model_id!r} from the local Hugging Face cache. "
                    "The vlarl DINOACT loader defaults to offline/local loading to avoid repeated "
                    "huggingface.co HEAD requests during RL runs. Download the model once, set "
                    "`dinov3_model_id` to a local snapshot directory, or run with "
                    "VLARL_DINOV3_LOCAL_FILES_ONLY=0 if you intentionally want network access."
                ) from e
            raise
...
class DINOACTRTCPolicy(PreTrainedPolicy):
    """
    Action Chunking Transformer Policy as per Learning Fine-Grained Bimanual Manipulation with Low-Cost
    Hardware (paper: https://huggingface.co/papers/2304.13705, code: https://github.com/tonyzhaozh/act)
    """

    config_class = DINOACTRTCConfig
    name = "dinoactrtc"
    ...
    def _unnormalize_action_dims(self, action: Tensor, indices: list[int]) -> Tensor:
        ...
        norm_mode = self.config.normalization_mapping.get(
            "ACTION", self.config.normalization_mapping.get(FeatureType.ACTION, NormalizationMode.IDENTITY)
        )
        if norm_mode == NormalizationMode.IDENTITY:
            return action[..., indices]
        if norm_mode == NormalizationMode.MEAN_STD and not getattr(self.config, "standardize_rot6d", True):
            return action[..., indices]
    ...
    def forward(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
        ...
        if self.config.use_recon_loss_mask:
            action_mask = (~batch["action_is_pad"]).to(dtype=actions_hat.dtype).unsqueeze(-1)
            if rtc_postfix_loss_mask is not None:
                # Training-time RTC: zero out the loss on positions corresponding to the action prefix
                # so the model is only supervised on the postfix it actually predicts.
                action_mask = action_mask * rtc_postfix_loss_mask.to(
        ...
        if self.config.use_rot6d_chordal_loss:
            ...
            rot_chordal = rot_chordal * action_mask
...
class DINOACTRTC(nn.Module):
    """Action Chunking Transformer with DINOv3 backbone: underlying network for DINOACTRTCPolicy.
    ...
    def __init__(self, config: DINOACTRTCConfig):
    ...
    def forward(
        ...
        Returns:
            ...
            latent dimension.
            Optional (B, chunk_size) boolean tensor with the training-time RTC postfix mask (True at
            positions that should contribute to the reconstruction loss). It is `None` when
            training-time RTC is disabled or the model is not in training mode.
        """
        ...
        if self.config.robot_state_feature:
            ...
            if self.training and self.config.proprio_noise_scale > 0:
                noise = torch.randn_like(robot_state)
                robot_state = robot_state + self.config.proprio_noise_scale * noise
            if self.training and self.config.proprio_dropout > 0:
                keep_mask = (
                    torch.rand(robot_state.shape[0], 1, device=robot_state.device)
                    > self.config.proprio_dropout
                ).to(dtype=robot_state.dtype)
        ...
        # Training-/Inference-Time RTC Tokens.
        # Following https://arxiv.org/abs/2512.05964: feed an action prefix as additional encoder
        # tokens so the model conditions on it when predicting the rest of the chunk. During
        # training, the prefix is taken from the ground-truth action chunk and `delay` is sampled
        # uniformly in [0, max_d]; the regression loss is masked so it is only applied to the
        # postfix. During inference, the caller can pass `batch["action_prefix"]` (and optionally
        # `batch["prefix_delay"]`) to provide a prefix from the previously committed chunk.
        ...
        if self.config.training_time_rtc:
            ...
            if self.training and ACTION in batch:
                # Training: random delay, ground-truth prefix sliced from the full action chunk.
                ...
                # Inference: caller-provided prefix (already in normalized action space).
                raw_prefix = batch["action_prefix"]
                device = raw_prefix.device
                # Pad / truncate the caller-provided prefix to exactly `max_d` slots.
                ...
                # Number of valid prefix slots per sample.
        ...
        if prefix_actions is not None:
            ...
            # Always feed `max_d` prefix tokens so the encoder sequence length is fixed across the
            # batch; invalid slots (>= delay) are masked out via `key_padding_mask`.
            prefix_embed = self.encoder_prefix_action_input_proj(prefix_actions)  # (B, max_d, D)
            prefix_embed = prefix_embed.transpose(0, 1)  # (max_d, B, D)
            # Reuse the decoder positional embedding for the matching chunk timesteps so each prefix
            # token carries the same positional code as the corresponding decoder query.
            prefix_pos = self.decoder_pos_embed.weight[:max_d].unsqueeze(1)  # (max_d, 1, D)
            ...
            prefix_validity = positions[None, :] < delay[:, None]  # (B, max_d), True = valid token
            ...
            if self.training:
                # Loss is computed on positions >= delay only; True marks postfix positions.
                chunk_positions = torch.arange(
                    self.config.chunk_size, device=prefix_actions.device
                )
