...
@PreTrainedConfig.register_subclass("dinoactrtc")
@dataclass
class DINOACTRTCConfig(ACTConfig):
    ...
    # If False, keep rot6d components in their original representation when STATE/ACTION use MEAN_STD
    # normalization. Position and tail dimensions are still normalized normally.
    standardize_rot6d: bool = True
    lambda_rot: float = 1.0

    #Training Time RTC Settings
    training_time_rtc: bool = True
    training_time_rtc_maxnum_actions: int = 5

    # If True, mask reconstruction loss with `action_is_pad` (and RTC postfix mask when enabled).
    # Default False: supervise all chunk timesteps equally.
    use_recon_loss_mask: bool = False
    # Training-time Gaussian noise scale for the proprioceptive state.
    proprio_noise_scale: float = 0.2
    # Training-time probability of zeroing the entire proprioceptive state for each sample.
    proprio_dropout: float = 0.25
    ...
    def __post_init__(self):
        ...
        if self.proprio_noise_scale < 0:
            raise ValueError("`proprio_noise_scale` must be non-negative.")
        if not 0.0 <= self.proprio_dropout <= 1.0:
            raise ValueError("`proprio_dropout` must be in [0.0, 1.0].")
