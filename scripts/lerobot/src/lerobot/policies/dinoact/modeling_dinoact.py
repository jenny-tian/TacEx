...
from lerobot.policies.dinoact.configuration_dinoact import DINOACTConfig
...
class DINOv3ACTBackbone(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "DINOACT requires the `transformers` package. Install with: pip install 'lerobot[transformers-dep]'"
            ) from e
...
class DINOACTPolicy(PreTrainedPolicy):
    """
    Action Chunking Transformer Policy as per Learning Fine-Grained Bimanual Manipulation with Low-Cost
    Hardware (paper: https://huggingface.co/papers/2304.13705, code: https://github.com/tonyzhaozh/act)
    """

    config_class = DINOACTConfig
    name = "dinoact"

    def __init__(
        self,
        config: DINOACTConfig,
        **kwargs,
    ):
        """
        Args:
...
class DINOACT(nn.Module):
    """Action Chunking Transformer with DINOv3 backbone: underlying network for DINOACTPolicy.
    ...
    def __init__(self, config: DINOACTConfig):
