...
# 每段 pos(3)+rot6d(6)；base 段后接 continuous cmd_vel（x 线速度、z 角速度）；最后 right_hand_closed。
...
class oli_roddy_loco(Robot):
    ...
    def __init__(self, config: oli_roddy_locoConfig):
        ...
        self.start_cmd_msg = self._load_start_cmd_from_bag(
            "/home/limx/dataset/pick_chip0525/012.bag"
        )
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        finger_cmd = np.array([action["right_hand_closed.pos"]], dtype=np.float32)
        ...
        # print(float(finger_cmd[0]))

        self._publish_brainco2_hand_cmd(closed=float(finger_cmd[0]) > 0.6)
    ...
    def to_start_pose(self, num_steps: int = 50, duration_s: float = 3.0) -> None:
        """回到 bag 起始 Teleop 位姿：从当前 hand pose 或 last_cmd_msg 插值过渡。"""
        start_msg = self.start_cmd_msg
        if start_msg is None:
            print("[WARN] start_cmd_msg is None; skip to_start_pose")
            return
        if self.last_cmd_msg is None:
            self.go_to_start_pose(num_steps=num_steps, duration_s=duration_s)
            return
