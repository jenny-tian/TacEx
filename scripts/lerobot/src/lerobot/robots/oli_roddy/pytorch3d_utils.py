...
# Device type for type hints (can be str or torch.device)
...
def make_action_relative_current_pose(item: dict,
                              current_pose_key: str = "observation.current_pose",
                              action_key: str = "action",) -> dict:

    base = item[current_pose_key]   # (T,8)
    actions = item[action_key] # (N,8)

    # T = current_pose.shape[0]
    N = actions.shape[0]

    base_pos = base[:, :3]
    base_rot6d = base[:, 3:9]  

    base_rotmat = rotation_6d_to_matrix(base_rot6d)
    base_mat = torch.eye(4, device=base.device).unsqueeze(0)  # (1,4,4)
    base_mat[:, :3, :3] = base_rotmat
    base_mat[:, :3, 3] = base_pos
    base_inv = torch.linalg.inv(base_mat)

    actions_pos = actions[:, :3]
    actions_rot6d = actions[:, 3:9]  # ✅ 6D
    actions_gripper = actions[:, 9:] # (50,1)

    actions_rotmat = rotation_6d_to_matrix(actions_rot6d)
    actions_mat = torch.eye(4, device=actions.device).unsqueeze(0).repeat(N, 1, 1)
    actions_mat[:, :3, :3] = actions_rotmat
    actions_mat[:, :3, 3] = actions_pos

    rel_actions_mat = base_inv @ actions_mat
    rel_actions_pos = rel_actions_mat[:, :3, 3]
    rel_actions_rotmat = rel_actions_mat[:, :3, :3]
    rel_actions_rot6d = matrix_to_rotation_6d(rel_actions_rotmat)

    item[action_key] = torch.cat(
        [rel_actions_pos, rel_actions_rot6d, actions_gripper],
        dim=-1
    )
    return item