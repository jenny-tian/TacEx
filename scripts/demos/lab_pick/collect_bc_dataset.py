...
parser.add_argument(
    "--labware_random_yaw_min_abs_degrees",
    type=float,
    default=0.0,
    help="Minimum absolute randomized yaw in degrees; signs remain uniformly sampled.",
)
...
def main():
    ...
    env_cfg.labware_yaw_randomization_min_abs = float(
        np.deg2rad(args_cli.labware_random_yaw_min_abs_degrees)
    )
    if env_cfg.labware_yaw_randomization_min_abs > env_cfg.labware_yaw_randomization:
        raise ValueError("Minimum absolute yaw cannot exceed the yaw randomization range.")
    ...
    collection_config = {
        ...
        "labware_random_yaw_min_abs_degrees": float(
            np.rad2deg(env_cfg.labware_yaw_randomization_min_abs)
        ),
        "seed": args_cli.seed,
    }
