...
深灰色结构为底板，浅灰色区域为正常表面，蓝色窄条为凹槽，红色方块为随机位置凸起。颜色使用不透明材质，便于在 Isaac Sim 视图和视频中识别。
...
4. 使用前方表面高度进行双向前馈：遇到凸起提前抬升，遇到凹槽提前下降。
5. 根据低通滤波后的力误差调整 Z 目标，并限制每步高度变化以减小刚性接触冲击。
...
平均力已接近 `3 N`，但尚未严格达到全程 `3±0.3 N`。凸起边缘和部分凹槽仍有瞬态误差；后续训练或控制器比较应同时报告 RMSE、容差带占比、失触率和峰值，不能只报告平均力。
