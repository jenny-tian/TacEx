...
def main() -> None:
    ...
    required = {
        "--policy": "Pass --policy /path/to/flow_matching.pt",
        "--checkpoint": "Pass --checkpoint /path/to/agent_N.pt",
    }
    for option, message in required.items():
        if not any(arg == option or arg.startswith(f"{option}=") for arg in sys.argv[1:]):
    ...
    command = [
        str(isaac_python),
        str(Path(__file__).with_name("eval_lab_pick_dsrl_sac_runtime.py")),
        *sys.argv[1:],
    ]
