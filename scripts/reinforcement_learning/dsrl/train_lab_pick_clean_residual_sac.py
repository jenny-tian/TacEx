#!/usr/bin/env python3
"""Launch the isolated clean LabPick residual-SAC trainer."""
...
def main() -> None:
    ...
    if not isaac_python_value:
        raise RuntimeError(
            "Set TACEX_ISAAC_PYTHON to the IsaacLab Python executable."
        )
    ...
    if not help_requested and not any(
    ...
    command = [
        ...
        str(
            ...
            / "skrl"
            / "train_clean_residual_sac.py"
        ),
        "--task",
        "TacEx-LabPick-Slide-Clean-Residual-SAC-v0",
        "--num_envs",
        "1",
        "--headless",
        "--enable_cameras",
