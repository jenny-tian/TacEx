...
`outputs/lab_pick_flow_bc200_hardyaw_rotation_scratch/best.pt`. Its weights are
frozen during SAC training. Physical-residual training starts with a gate of
`0.025`, a `0.02` floor, temperature `0.5`, correction penalty `5.0`, and
maximum gate `0.2`.
`DSRL/gate_mean`, `DSRL/gate_penalty`, and `DSRL/correction_rms` are written to
the normal SKRL/IsaacLab logs. Set `TACEX_DSRL_GATE_INIT`,
`TACEX_DSRL_GATE_TEMPERATURE`, `TACEX_DSRL_GATE_PENALTY`, or
`TACEX_DSRL_GATE_MAX` to override the defaults.

An unsuccessful time limit is terminal in SAC replay even though Gym reports it
as a truncation. This prevents target-Q from bootstrapping through Isaac Lab's
automatic reset observation. The default task rewards are `+120` for success,
`-30` for other terminal failures, and `-40` for timeout. SAC uses a macro-step
discount of `0.99`; rewards inside a decoded action chunk are summed without a
second discount. Frozen-BC Flow Matching noise is fixed within an episode and
resampled between episodes. The residual observation also contains normalized
remaining time. After half of the episode, incomplete grasp/lift progress gets
a gradually increasing dense penalty. Replay batches contain at least 25%
terminal samples so success and failure signals are not diluted by long episodes.
